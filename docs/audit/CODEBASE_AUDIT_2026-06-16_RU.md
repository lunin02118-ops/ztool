...
> решение проблемы тестирования у исполнителя). Новые находки **F-13** (окружение
> тестирования) и **F-14** (анонимные CommandManager-вкладки) добавлены ниже.
>
> **Рев. 3.1 (2026-06-17).** PR #33 обновлён (commit `2f0c9a4`): получено **живое SW-доказательство
> паритета**: `PROP-WRITE-ZTOOL` PASS (COM read-back подтвердил запись свойства), `BOM-07/08`,
> `REG-08/09` PASS, чистый пакет `104028` прошёл `verify_release_package.ps1`. Решение пользователя:
> **в продакшен идёт текущая версия** (бинарь), полная реконструкция (Трек 2) — фоном. См. раздел **R**
> в плане (релиз-чеклист текущей версии). B2 переведён в «частично закрыто».
...
release-candidate-ready. Клиентская часть на 2026-06-17 имеет **живое SW-доказательство
паритета** (PR #33: `PROP-WRITE-ZTOOL`, `BOM-07/08`, `REG-08/09` PASS, чистый пакет `104028`),
но это **ручное evidence одного прогона**, а не автоматический parity-гейт в CI. Поэтому для
**собственного продакшена текущей версии** статус — «готово при прохождении релиз-чеклиста
(раздел R: живой прогон + чистый манифест + деплой сервера лицензий)»; для **долгосрочной
поддерживаемости** остаётся в силе «client source not reconstructed» (Трек 2).

**Топ-блокеры до продакшена** (детали в §6 и в плане рефакторинга; статус на 2026-06-17):
- B1. ✅ Memory-dump в LFS убран из HEAD (PR #34) — чистый clone чинится.
- B2. ⚠️ Частично закрыто: есть живое SW-evidence паритета (PR #33), но нет авто-гейта в CI.
- B3. Phase 11 / готовые PR (#33/#34/#35) ещё не слиты в `main` — слить по порядку (см. план §B).
- F-14. ⚠️ «Пустая вкладка» ZTool: установщик/деинсталлятор не чистят анонимные CommandManager-вкладки.
...
> Поскольку главная цель — именно «полный исходный код с лёгкой поддержкой», **Вариант B
> расписан как основной (Трек 2)** в `docs/audit/REFACTORING_PLAN_2026-06-16_RU.md`:
> целевая архитектура solution, карта форм/классов (`Frmmain`, `FrmOptions`, `SwAddin`,
> `PMPHandler`, `CConfigDO`…), стратегия декомпиляции→чистый C#, реконструкция по подсистемам
> (чтение SW, свойства ЕСКД, BOM 8/8, печать/PDF/переименование/рамка, лицензирование, апдейтер),
> локализация (команды EN / UI RU+EN через `.resx`), тест-стратегия (включая SolidWorks-parity),
> CI/инсталлятор, вехи M0–M6 и оценка трудозатрат.
...
| B2 | P0→P1 | **Частично закрыто (PR #33, commit `2f0c9a4`):** есть живое SW-доказательство паритета, но нет авто-гейта в CI | `PROP-WRITE-ZTOOL` PASS (COM read-back), `BOM-07/08` strict `Тип` PASS, `REG-08/09` PASS; evidence в `manual-test-reports/FULL_TEST_20260617-methodology-final/` | Для релиза текущей версии — достаточно (раздел R). Для Трека 2 — довести `client-semantic-parity` до авто-гейта |
...
| F-14 | P1 | «Пустая вкладка» ZTool — анонимные CommandManager-вкладки не чистятся | PR #33 evidence: `AssyContext\Tab12`, `DrwContext\Tab10`, `PartContext\Tab21` без `ModuleName`/`RefName`, но с ZTool-кнопками (`2,59425`, `41658..41675`); старая очистка по `ZTool`/`59959DFA` их пропускала | Установщик/деинсталлятор и pre-flight должны чистить эту анонимную сигнатуру (план R1.1). DoD: ровно одна вкладка после установки |
