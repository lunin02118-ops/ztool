...
| R-013 | P1 | Cleaned production profile/package can lose the default material library and material-color behavior (`materialpath` empty, `usematerialcolor=false`, missing `SW模板`). | Mitigated in Phase 10 follow-up: release package copies `SW模板/`, normalizes `materialpath` to packaged `MyMaterials.sldmat`, restores `usematerialcolor=true`, and verifier fails if the material library gate is broken. Manual SolidWorks material/color smoke remains required. | Phase 10 |
