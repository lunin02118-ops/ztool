...
| R-009 | P0 | Корневые `ZTool.exe`/`ZTool.dll` не совпадают с live-tested recommended artifacts. | Mitigated in Phase 10 and updated after the material/color key fix: root `ZTool.exe` is `C5785471...2DD03A9`, root `ZTool.dll` is `pmpguard2` (`D0535425...0E492EB9`), and release packager now uses root runtime artifacts by default. Final SolidWorks acceptance still required on the release package. | Phase 10/11 |
