...
8. Прогнать «Разделить столбец» и удалить тестовую строку клавишей `Delete`
   (`REG-09`).
9. Экспортировать все 8 режимов BOM в отдельную папку.
10. Запустить `validate_bom_exports.py` на папку с экспортами.
11. Выполнить цветовые A/B проверки §6.3 против оригинала.
12. Проверить Event Viewer: нет новых `Application Error`, `.NET Runtime`, WER/dump.

Если любой из шагов 1–12 пропущен, итоговый статус — не `FULL PASS`, а
