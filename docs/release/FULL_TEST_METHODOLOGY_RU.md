...
> Запуск ZTool **только из ленты SolidWorks** («Управление файлами» стартует
> `runtime/ZTool.exe`), не напрямую — иначе часть путей/контекста SW недоступна.
...
| ID | Шаги | Ожидаемо |
|----|------|----------|
| RD-01 | Открыть `0614-A00`, лента → «Управление файлами» | Стартует `runtime/ZTool.exe`, лента/иконки видны |
...
| ID | Шаги | Ожидаемо |
|----|------|----------|
| PROP-01 | Сверить `propname`/`namemapping` (RU) с колонками экспорта | Колонки `Наименование/Обозначение/Материал/Тип/Версия/Обработка поверхности/Масса` подписаны и заполнены из свойств модели |
