...
Запрещено считать `SendKeys("^v")` доказательством ввода пути в `SaveDialog`:
в live-прогонах 2026-06-22 это приводило к буквальному `v.xlsx` и конфликтам
перезаписи. Надежный способ для автоматизации:

- найти `SaveDialog` как окно `#32770`;
- поле имени файла искать как `ControlType.Edit`, `ClassName=Edit`,
  `AutomationId=1001`;
- задать полный путь через UIA `ValuePattern.SetValue`;
- нажать кнопку сохранения через `InvokePattern` по `AutomationId=1`;
- вопрос `Экспорт выполнен! Открыть?` закрывать кнопкой `Нет`
  (`AutomationId=7`) или, если legacy message box не отдаёт кнопки как
  descendants, кликом внутри найденного окна `Вопрос` с обязательным скриншотом
  и записью причины fallback.

Для dropdown `Экспорт спецификации` в WinForms/Ribbon допускается один
координатный fallback только на открытие меню, если ribbon-кнопка не отдаётся
UIA. Сами режимы выбирать не по координатам строк, а по найденным UIA
`MenuItem` в открытом dropdown, отсортированным по `BoundingRectangle.Top`.
Иначе длинные пункты меню могут сдвинуть Y-координаты и тест незаметно
экспортирует не тот режим.
...
Инцидент 2026-06-22 (`SWTools 1.1.6`): экспорт через правильный маршрут
SolidWorks -> `Управление файлами` -> `Подключить SW` -> `Экспорт спецификации`
создал все 8 `.xlsx`, но `validate_bom_exports.py` вернул `FAIL`: расчетные
колонки `Масса ед. кг` и `Габаритные размеры` были `0/N` в режимах 1-6. Такой
результат нельзя принимать как частичный успех: если `№ п/п`, `Кол-во` и `Путь`
заполнены, но `Col_Weight`/`Col_bound` пустые, это release blocker. Проверять
нужно именно Excel read-back, а не факт появления файла и не сообщение
`Экспорт выполнен`.

Решение инцидента 2026-06-22: from-source export должен резолвить `mappingname`
не только для `PropVal_*`, но и для постоянных расчетных `Col_*`. Обязательная
проверка после исправления:

- `FrmMapping` показывает `Масса ед._кг` и `Габаритные размеры`;
- при старом/поврежденном installed `SWTools.settings` конфиг self-heal
  восстанавливает `Col_Weight -> МассаЕдКг` и
  `Col_bound -> ГабаритныеРазмеры` в памяти;
- `validate_bom_exports.py` по 8 реальным `.xlsx` возвращает `PASS/WARN`, где
  `BOM-01`...`BOM-06` имеют `Масса` и `Габариты = N/N`;
- предупреждения допустимы только для `BOM-07/08` с 0 строк на модели без
  заполненного `Тип`; для `FULL PASS` нужен отдельный fixture с непустыми
  фильтровыми результатами.
...
11. Для режимов `BOM-01`...`BOM-06` обязательны непустые расчетные колонки:
    `Масса ед. кг` (`Col_Weight`/`МассаЕдКг`) и `Габаритные размеры`
    (`Col_bound`/`ГабаритныеРазмеры`). `0/N` по любой из них = `NO-GO`.
12. Для `FULL PASS` закрыть строгий fixture gate `BOM-07`/`BOM-08`: оба файла
    непустые, фильтр применён, столбец `Тип` не содержит пустых значений.
13. Выполнить цветовые A/B проверки §6.3 против оригинала. Для текущего релиза
    release gate — `COL-UI`; `COL-XLSX` принят out of scope.
14. Проверить Event Viewer: нет новых `Application Error`, `.NET Runtime`, WER/dump.

Если любой из шагов 1–14 пропущен, итоговый статус — не `FULL PASS`, а
