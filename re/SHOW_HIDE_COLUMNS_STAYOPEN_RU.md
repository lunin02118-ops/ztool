...
Документ основан на анализе реального кода. Каноничный исходник клиента теперь
живёт из исходников в `client-src/ZTool/Frmmain.cs`. Декомпилированный
референс RibbonLib (`Ribbon.dll`), на который ссылается раздел про API, лежал во
временной ветке закрытого PR #43 — RibbonLib это сторонняя библиотека и в репозиторий
не коммитится. Номера строк ниже приблизительны (снимок декомпиляции).

## 1. Что это за контрол (по факту из кода)

В `client-src/ZTool/Frmmain.cs`:
...
В управляемом коде ручки для этого нет: `RibbonLib.Controls/RibbonDropDownGallery.cs` (сторонний RibbonLib)
...
- `RibbonLib.Controls/RibbonDropDownGallery.cs` (сторонний RibbonLib) —
