...
public class FrmFilterrules : Form
{
	...
	private virtual CustomComboBox2 ComboBox1
	{
		[DebuggerNonUserCode]
		get
		{
	...
	private void InitializeComponent()
	{
		...
		this.Label1.Text = "Del на заголовке строки удаляет всю строку.\r\nНесколько значений разделяйте латинской точкой с запятой.";
		...
		this.Controls.Find("GroupBox1", true)[0].Width = 220;
		this.Controls.Find("RuleNameList", true)[0].Width = 202;
		this.Controls.Find("TableLayoutPanel2", true)[0].Width = 204;
		this.Controls.Find("add", true)[0].Text = "Доб.";
		this.Controls.Find("edit", true)[0].Text = "Изм.";
		this.Controls.Find("del", true)[0].Text = "Удал.";
		this.Controls.Find("GroupBox2", true)[0].Left = 250;
		this.Controls.Find("GroupBox2", true)[0].Width = 560;
		this.Controls.Find("DGV1", true)[0].Width = 540;
		this.Controls.Find("Label1", true)[0].Width = 540;
	}
	...
	public void Refreshrulelist()
	{
		try
		{
			int num = RulesList.FindIndex([SpecialName] (CustomRule s) => s.name.Equals(RuleName, StringComparison.OrdinalIgnoreCase));
			if (num >= 0)
			{
	...
	private void edit_Click(object sender, EventArgs e)
	{
		...
		checked
		{
			try
			{
				...
				{
					canchange = false;
					string right = Conversions.ToString(RuleNameList.Items[RuleNameList.SelectedIndex]);
					RuleNameList.Items[RuleNameList.SelectedIndex] = text;
					RuleName = text;
					RulesList[RuleNameList.SelectedIndex] = CreateCustomRule(RuleName);
					int num4 = CConfigMng.Config.bomsettings.FindIndex([SpecialName] (bomsetting s) => Operators.CompareString(s.name, right, TextCompare: false) == 0);
					if (num4 >= 0)
					{
						CConfigMng.Config.bomsettings[num4].name = text;
					}
					int num5 = CConfigMng.Config.ReNameFilterRule.FindIndex([SpecialName] (string s) => Operators.CompareString(s, right, TextCompare: false) == 0);
					if (num5 >= 0)
					{
