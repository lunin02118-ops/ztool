...
public class FrmFilling : Form
{
					...
					if (MyProject.Forms.Frmmain.DGV1.Rows[num8].Visible)
					{
						...
						if (Operators.CompareString(text3, "", TextCompare: false) != 0)
						{
							...
							while (true)
							{
								...
								{
									string text4 = "<" + MyProject.Forms.Frmmain.DGV1.Columns[num11].HeaderText + ">";
									if (vallist.Exists([SpecialName] (string s1) => s1.Equals(text4, StringComparison.OrdinalIgnoreCase)))
									{
										text3 = Strings.Replace(text3, text4, Conversions.ToString(MyProject.Forms.Frmmain.DGV1[num11, num8].Value));
									}
								}
								num11++;
							}
							string text5 = code.MidStrEx(text3, "{", "}");
							if (Versioned.IsNumeric(text5))
							{
								text3 = Strings.Replace(text3, "{" + text5 + "}", (Conversions.ToDouble(text5) + (double)num5).ToString().PadLeft(text5.Length, '0'));
							}
						}
						...
						if (((customFilter.FilterByRule(num3) && fillsetting2.colindex >= 0 && fillsetting2.colindex <= MyProject.Forms.Frmmain.DGV1.ColumnCount - 1) ? true : false) && ((!MyProject.Forms.Frmmain.DGV1[fillsetting2.colindex, num3].ReadOnly && fillsetting2.colindex != MyProject.Forms.Frmmain.Col_FileName.Index) ? true : false))
						{
							...
							if (Operators.CompareString(text2, "", TextCompare: false) != 0)
							{
								...
								while (true)
								{
									...
									{
										string text3 = "<" + MyProject.Forms.Frmmain.DGV1.Columns[num13].HeaderText + ">";
										if (vallist.Exists([SpecialName] (string s1) => s1.Equals(text3, StringComparison.OrdinalIgnoreCase)))
										{
											text2 = Strings.Replace(text2, text3, Conversions.ToString(MyProject.Forms.Frmmain.DGV1[num13, num3].Value));
										}
									}
									num13++;
								}
	...
	private void add_Click(object sender, EventArgs e)
	{
		...
		int num = CConfigMng.Config.fillsettings.FindIndex([SpecialName] (fillsetting s) => s.name.Equals(text, StringComparison.OrdinalIgnoreCase));
		if (num >= 0)
		{
			MessageBox.Show(this, "Имя «" + text + "» уже существует, выберите другое имя", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
			return;
		}
		fillsettinglist.Items.Add(text);
		CConfigMng.Config.fillsettings.Add(createfillsetting(text));
	...
	private void edit_Click(object sender, EventArgs e)
	{
		...
		{
			text = fillsettinglist.SelectedItem.ToString();
		}
		int num = CConfigMng.Config.fillsettings.FindIndex([SpecialName] (fillsetting s) => s.name.Equals(text, StringComparison.OrdinalIgnoreCase));
		if (num < 0)
		{
		...
		{
			int num2 = CConfigMng.Config.fillsettings.FindIndex([SpecialName] (fillsetting s) => s.name.Equals(text2, StringComparison.OrdinalIgnoreCase));
			if (num2 < 0)
			{
				fillsetting fillsetting2 = CConfigMng.Config.fillsettings[num];
				fillsetting2.name = text2;
				CConfigMng.Config.fillsettings[num] = fillsetting2;
				fillsettinglist.Items[fillsettinglist.SelectedIndex] = text2;
			}
			else
			{
	...
	private void fillsettinglist_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		checked
		{
			...
			int num3 = 0;
			_Closure_0024__83._Closure_0024__84 closure_0024__ = default(_Closure_0024__83._Closure_0024__84);
			while (true)
			{
				int num4 = num3;
				int num5 = num2;
				...
				closure_0024__ = new _Closure_0024__83._Closure_0024__84(closure_0024__);
				closure_0024__._0024VB_0024Local_name = Conversions.ToString(RuleList.Items[num3]);
				int num6 = fillsetting2.RulesList.FindIndex(closure_0024__._Lambda_0024__142);
				if (num6 >= 0)
				{
	...
	public void updatefillsetting()
	{
		...
		checked
		{
			...
			while (true)
			{
				...
				{
					text = fillsettinglist.Items[num2].ToString();
					break;
				}
				num2++;
			}
			if (Operators.CompareString(text.Trim(), "", TextCompare: false) != 0)
			{
				int num5 = CConfigMng.Config.fillsettings.FindIndex([SpecialName] (fillsetting s) => s.name.Equals(text, StringComparison.OrdinalIgnoreCase));
				if (num5 >= 0)
				{
					CConfigMng.Config.fillsettings[num5] = createfillsetting(text);
					CConfigMng.SaveConfig();
				}
			}
		}
