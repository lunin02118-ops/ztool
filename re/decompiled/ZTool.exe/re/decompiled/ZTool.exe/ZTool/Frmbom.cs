...
public class Frmbom : Form
{
	...
	private void ToolStripDropDownButton1_DropDownItemClicked(object sender, ToolStripItemClickedEventArgs e)
	{
		try
		{
			...
			bomsetting bst = CConfigMng.Config.bomsettings.Find([SpecialName] (bomsetting s) => s.name.Equals(e.ClickedItem.Text, StringComparison.OrdinalIgnoreCase));
	...
	public TreeGridNode addnodedata(Treenode node, TreeGridNodeCollection gridnodes, int level)
	{
		...
		{
			if ((Convert.ToString(RuntimeHelpers.GetObjectValue(gridnode.Tag)).Equals(text, StringComparison.OrdinalIgnoreCase) && Operators.CompareString(text, "", TextCompare: false) != 0) ? true : false)
			{
				gridnode.Cells[2].Value = Operators.AddObject(gridnode.Cells[2].Value, 1);
				return null;
			}
		}
		Treenode treenode = ((!code.togetherConfig) ? Clist.Find([SpecialName] (Treenode s) => text.Equals(s.PathName + "\n" + s.ConfigureName, StringComparison.OrdinalIgnoreCase)) : Clist.Find([SpecialName] (Treenode s) => text.Equals(s.PathName, StringComparison.OrdinalIgnoreCase)));
		checked
		{
