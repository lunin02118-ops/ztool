...
public class Frmmain : Form
{
	...
	private virtual CustomComboBox1 ComboBox1
	{
		[DebuggerNonUserCode]
		get
		{
	...
	private virtual BackgroundWorker GetAsmBgWorker
	{
		[DebuggerNonUserCode]
		get
		{
	...
	private virtual BackgroundWorker SaveToSWBgWorker
	{
		[DebuggerNonUserCode]
		get
		{
	...
	public void loadfrm()
	{
		...
		checked
		{
			try
			{
				...
				IsStop.MouseLeave += [SpecialName] [DebuggerStepThrough] (object a0, EventArgs a1) =>
				{
				...
				PropSwitch.MouseLeave += [SpecialName] [DebuggerStepThrough] (object a0, EventArgs a1) =>
				{
					ToolStripButton_MouseLeave();
				};
				...
				AutoColumnsMode.Tag = "Автоширина";
				AutoColumnsMode.Alignment = ToolStripItemAlignment.Right;
				AutoColumnsMode.MouseHover += ToolStripButton_MouseHover;
				AutoColumnsMode.MouseLeave += [SpecialName] [DebuggerStepThrough] (object a0, EventArgs a1) =>
				{
					ToolStripButton_MouseLeave();
				};
				...
				PreviewSwitch1.MouseLeave += [SpecialName] [DebuggerStepThrough] (object a0, EventArgs a1) =>
				{
					ToolStripButton_MouseLeave();
				};
	...
	public void button4_ExecuteEvent(object sender, ExecuteEventArgs e)
	{
		...
		try
		{
			bomsetting bst = CConfigMng.Config.bomsettings.Find([SpecialName] (bomsetting s) => s.name.Equals(ribbonButton.Label, StringComparison.OrdinalIgnoreCase));
			...
			else if (LastCustomFilterButton.CommandID == _ExcludeBom.CommandID)
			{
				...
				if (list.Count > 0)
				{
					...
					while (true)
					{
						...
						{
							string text2 = Conversions.ToString(DGV1[Col_Path.Index, num21].Value);
							flag = list.Exists([SpecialName] (Treenode s) => text2.Equals(s.PathName, StringComparison.OrdinalIgnoreCase));
						}
						else
						{
							string text3 = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(DGV1[Col_Path.Index, num21].Value, "\n"), DGV1[Col_Cfg.Index, num21].Value));
							flag = list.Exists([SpecialName] (Treenode s) => text3.Equals(s.PathName + "\n" + s.ConfigureName, StringComparison.OrdinalIgnoreCase));
						}
						if (!flag)
						{
	...
	private void DGV1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
	{
		...
		else if ((sel_col == Col_Drw.Index) & CConfigMng.Config.DClick_OpenInSw)
		{
			...
			{
				Thread thread2 = new Thread([SpecialName] [DebuggerStepThrough] (object a0) =>
				{
					code.OpenDoc(Conversions.ToString(a0));
				});
				thread2.Start(text2);
		...
		checked
		{
			...
			while (true)
			{
				...
				else
				{
					...
					if (code.togetherConfig)
					{
						value = Conversions.ToString(DGV1[Col_Path.Index, num3].Value);
					}
					else
					{
						value = Conversions.ToString(Operators.ConcatenateObject(DGV1[Col_Path.Index, num3].Value, DGV1[Col_Cfg.Index, num3].Value));
					}
					if (nodelist.Exists([SpecialName] (string s) => s.Equals(value, StringComparison.OrdinalIgnoreCase)))
					{
						num++;
						DGV1[Col_Checkbox.Index, num3].Value = true;
					}
	...
	public void getnodes(Treenode pnode, ref List<string> nodelist, bool onlytop, int Level = 0)
	{
		try
		{
			...
			string text = Conversions.ToString(Interaction.IIf(code.togetherConfig, pnode.PathName, pnode.PathName + pnode.ConfigureName));
			if (!nodelist.Exists([SpecialName] (string s) => s.Equals(text, StringComparison.OrdinalIgnoreCase)) && (!onlytop || ((onlytop && Level > 0) ? true : false)))
			{
				nodelist.Add(text);
			}
	...
	public void getnodes2(Treenode pnode, ref List<string> nodelist, bool Includemyself = false, int Level = 0)
	{
		try
		{
			...
			string text = Conversions.ToString(Interaction.IIf(code.togetherConfig, pnode.PathName, pnode.PathName + pnode.ConfigureName));
			if (!nodelist.Exists([SpecialName] (string s) => s.Equals(text, StringComparison.OrdinalIgnoreCase)) && (Includemyself || ((!Includemyself && Level > 0) ? true : false)))
			{
				nodelist.Add(text);
			}
	...
	private void GetMaterialName()
	{
		...
		checkBox.CheckedChanged += [SpecialName] [DebuggerStepThrough] (object a0, EventArgs a1) =>
		{
		...
		checked
		{
			try
			{
				...
				else if (m.Msg == 74)
				{
					...
					{
						string[] array = Strings.Split(cOPYDATASTRUCT.lpData, "\r\n");
						if (array.Length < 4)
						{
							return;
						}
						if (code.Cbool1(array[0]))
						{
							int num = DGV1.RowCount - 1;
							int num2 = 0;
							while (true)
							{
								...
								if (string.Equals(Conversions.ToString(DGV1[Col_Path.Index, num2].Value), array[1], StringComparison.OrdinalIgnoreCase))
								{
									DGV1[Col_Path.Index, num2].Value = array[2];
							...
							while (true)
							{
								...
								if (text.Equals(array[1], StringComparison.OrdinalIgnoreCase))
								{
									listView.Items[num6].SubItems[1].Text = array[2];
								}
								num6++;
							}
							listView = null;
							refreshnodename(TV1.Nodes, array[1], array[2]);
						}
						int num8 = (int)Math.Round(Conversion.Val(array[3]));
						if (array.Length < 5)
						{
							return;
						}
						array = array.Where([SpecialName] (string s) => array.ToList().IndexOf(s) > 3).ToArray();
						if (array.Length > 0)
						{
							...
							MyProject.Forms.Frmtips.additem(1, Conversions.ToString(DGV1[Col_Number.Index, num8].Value), string.Join("；", array), "Сохранить в SW", Conversions.ToString(DGV1[Col_Path.Index, num8].Value));
						}
					}
					...
					else if (dwData == (IntPtr)1000)
					{
						...
						BeginInvoke((VB_0024AnonymousDelegate_2)([SpecialName] () =>
						{
							BringToFrontSafely();
						}));
			...
			try
			{
				...
				while (true)
				{
					...
					try
					{
						...
						{
							string headerText = DGV1.Columns[num5].HeaderText;
							string name = DGV1.Columns[num5].Name;
							string text2 = "";
							columnnamemapping columnnamemapping2 = null;
							if (DGV1.Columns[num5].Name.Contains("PropVal_"))
							{
								columnnamemapping2 = CConfigMng.Config.namemappinglist.Find([SpecialName] (columnnamemapping s) => (s.text.Equals(headerText, StringComparison.OrdinalIgnoreCase) && s.name.Equals(name, StringComparison.OrdinalIgnoreCase)) ? true : false);
							}
							else if (DGV1.Columns[num5].Name.Contains("PropResolvedVal_"))
							{
								columnnamemapping2 = CConfigMng.Config.namemappinglist.Find([SpecialName] (columnnamemapping s) => (s.text.Equals(headerText, StringComparison.OrdinalIgnoreCase) && s.name2.Equals(name, StringComparison.OrdinalIgnoreCase)) ? true : false);
							}
		...
		checked
		{
			if ((bst.type == 0 || bst.type == 2 || bst.type == 3) ? true : false)
			{
				...
				while (true)
				{
					...
					list2.Add(Conversions.ToString(DGV1[Col_Path.Index, num2].Value));
					num2++;
				}
				list = list.OrderBy([SpecialName] (Treenode s) => list2.IndexOf(s.PathName)).ToList();
			}
			SaveFileDialog saveFileDialog = new SaveFileDialog();
			saveFileDialog.DefaultExt = Path.GetExtension(bomname);
			...
			try
			{
				...
				while (true)
				{
					...
					try
					{
						...
						{
							string headerText = DGV1.Columns[num8].HeaderText;
							string name = DGV1.Columns[num8].Name;
							string text2 = "";
							columnnamemapping columnnamemapping2 = null;
					...
					while (true)
					{
						...
						if (num15 != -1 && (((bst.type != 0 || !bst.ByRuler || Information.IsNothing(customFilter)) && 0 == 0) || customFilter.FilterByRule(num15)) && (((bst.type != 0 || !bst.ByFilter) && 0 == 0) || DGV1.Rows[num15].Visible))
						{
							...
							while (true)
							{
								...
								if (list4[num17] == Col_Number.Index)
								{
									if (bst.type == 1)
									{
										NewLateBinding.LateSet(objectValue3, null, "cells", new object[3]
								...
								else if ((list4[num17] == Col_Preview.Index && bst.insertimagebool) ? true : false)
								{
									if (!Information.IsNothing(RuntimeHelpers.GetObjectValue(DGV1[list4[num17], num15].Value)))
									{
										object instance6 = objectValue3;
										array = new object[2]
										{
										...
										if (array2[1])
										{
											list5[index] = (int)Conversions.ChangeType(RuntimeHelpers.GetObjectValue(array3[1]), typeof(int));
										}
										object instance7 = obj2;
										...
										instance7 = null;
										Image image = new Bitmap((Image)DGV1[list4[num17], num15].Value, 256, 192);
								...
								else if (list4[num17] == Col_Quantity.Index)
								{
									if (bst.type == 1)
									{
										NewLateBinding.LateSet(objectValue3, null, "cells", new object[3]
								...
								else if (list4[num17] == Col_Level.Index)
								{
									NewLateBinding.LateSet(objectValue3, null, "cells", new object[3]
									{
										num10 + num11,
								...
								else if ((list4[num17] != Col_Number.Index && list4[num17] != Col_Preview.Index && list4[num17] != Col_Quantity.Index) ? true : false)
								{
									NewLateBinding.LateSet(objectValue3, null, "cells", new object[3]
									{
										num10 + num11,
										list3[num17],
										RuntimeHelpers.GetObjectValue(DGV1[list4[num17], num15].Value)
									}, null, null);
									flag = true;
								}
								num17++;
			...
			try
			{
				...
				while (true)
				{
					...
					try
					{
						...
						{
							string headerText = DGV1.Columns[num3].HeaderText;
							string name = DGV1.Columns[num3].Name;
							string text = "";
							columnnamemapping columnnamemapping2 = null;
							if (DGV1.Columns[num3].Name.Contains("PropVal_"))
							{
							...
							IName name2 = workbook.GetName(text);
							if (!Information.IsNothing(name2))
							{
								CellReference cellReference = new CellReference(name2.RefersToFormula);
			...
			try
			{
				...
				while (true)
				{
					...
					try
					{
						...
						{
							string headerText = DGV1.Columns[num6].HeaderText;
							string name = DGV1.Columns[num6].Name;
							string text = "";
							columnnamemapping columnnamemapping2 = null;
				...
				if (list3.Count >= 1)
				{
					...
					while (true)
					{
						...
						{
							bool flag2 = false;
							mynpoi.InsertRows(ref sheet, row + num8, 1, list3);
							...
							if (!Information.IsNothing(row2))
							{
								...
								while (true)
								{
									...
									cell = row2.GetCell(list3[num14]);
									if (Information.IsNothing(cell))
									{
										cell = row2.CreateCell(list3[num14]);
									}
									if (list4[num14] == Col_Number.Index)
									{
										if (bst.type == 1)
										{
									...
									else if ((list4[num14] == Col_Preview.Index && bst.insertimagebool) ? true : false)
									{
										col_exclude = list3[num14];
										if (!Information.IsNothing(RuntimeHelpers.GetObjectValue(DGV1[list4[num14], num12].Value)))
										{
											row2.Height = (short)(image_height * 20);
											sheet.SetColumnWidth(list3[num14], (int)Math.Round(((double)image_width / 6.0 + 0.56) * 256.0));
											Image image = new Bitmap((Image)DGV1[list4[num14], num12].Value, 256, 192);
											byte[] pictureData = ImageHelper.ImageToBytes(image);
											int pictureIndex = workbook.AddPicture(pictureData, PictureType.PNG);
											if (flag)
											{
												HSSFPatriarch hSSFPatriarch = (HSSFPatriarch)sheet.CreateDrawingPatriarch();
												HSSFClientAnchor anchor = new HSSFClientAnchor(20, 5, 1003, 250, list3[num14], row + num8, list3[num14], row + num8);
											...
											else
											{
												...
												XSSFClientAnchor anchor2 = new XSSFClientAnchor(9000, 9000, 0, 0, list3[num14], row + num8, list3[num14] + 1, row + num8 + 1);
												XSSFPicture xSSFPicture = (XSSFPicture)xSSFDrawing.CreatePicture(anchor2, pictureIndex);
											}
										}
										flag2 = true;
									}
									else if (list4[num14] == Col_Quantity.Index)
									{
									...
									else if (list4[num14] == Col_Level.Index)
									{
										cell.SetCellValue(list[num10].mylevel);
										flag2 = true;
									}
									else if ((list4[num14] != Col_Number.Index && list4[num14] != Col_Preview.Index && list4[num14] != Col_Quantity.Index) ? true : false)
									{
										string text2 = Conversions.ToString(DGV1[list4[num14], num12].Value);
										if (long.TryParse(text2, out var result))
										{
											cell.SetCellValue(result);
										}
					...
					while (true)
					{
						...
						if (num11 != -1)
						{
							...
							while (true)
							{
								...
								stringBuilder2.Append(Operators.ConcatenateObject("\t", DGV1[list3[num13], num11].Value));
								num13++;
							}
