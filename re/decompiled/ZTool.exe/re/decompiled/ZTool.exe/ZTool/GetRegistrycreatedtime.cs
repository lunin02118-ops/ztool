...
internal class GetRegistrycreatedtime
{
	...
	private static long GetRegistryKeyValue(RootName rootKeyName, string subKeyName, int index)
	{
		...
		RegCreateKeyEx(hKey, subKeyName, IntPtr.Zero, null, RegOption.NonVolatile, RegSAM.EnumerateSubKeys | RegSAM.WOW64_64Key, ref lpSecurityAttributes, ref phkResult, ref lpdwDisposition);
		uint lpcbName = 100u;
		StringBuilder lpName = new StringBuilder();
		long lpftLastWriteTime = 0L;
		uint index2 = checked((uint)index);
		RegEnumKeyEx(phkResult, index2, lpName, ref lpcbName, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, ref lpftLastWriteTime);
		if (phkResult != UIntPtr.Zero)
		{
			GetRegistrycreatedtime.RegCloseKey(phkResult);
		}
					...
					if (subKeyNames.Length != 0)
					{
						...
						while (true)
						{
							...
							if (num3 <= num4)
							{
								...
								{
									result = DateTime.FromFileTime(GetRegistryKeyValue(rootKeyName, KeyName, num2)).ToString("yyyy/M/d HH:mm:ss", DateTimeFormatInfo.InvariantInfo);
								}
								num2++;
								continue;
							}
