...
public class MySWDM
{
	...
	internal List<string> GetPropertyNames1()
	{
		...
		{
			int num;
			if (Strings.InStr(Strings.LCase(text2), "sldprt") > 0)
			{
				num = 1;
			}
			else
			{
				if (Strings.InStr(Strings.LCase(text2), "sldasm") <= 0)
				{
					if (Strings.InStr(Strings.LCase(text2), "slddrw") > 0)
					{
						num = 3;
					}
					else
			...
			try
			{
				...
				SwDMDocument document = swDMApplication.GetDocument(text2, (SwDmDocumentType)docType, allowReadOnly: true, out result);
				num2 = (int)result;
				...
				foreach (object item in (IEnumerable)objectValue)
				{
					...
					if (!list.Exists([SpecialName] (string s) => s.Equals(text, StringComparison.OrdinalIgnoreCase)))
					{
						list.Add(text);
					}
				}
	...
	internal List<string> GetPropertyNames2()
	{
		...
		{
			text2 = fileBorser.DirectoryPath;
		}
		...
		code.SearchFiles(list2, text2, flag2, text3, flag);
			...
			try
			{
				...
				while (objectValue != null)
				{
					...
					for (int i = 0; i < array2.Length; i++)
					{
						...
						{
							text3 = array6[j];
							if (!list.Exists([SpecialName] (string s) => s.Equals(text3, StringComparison.OrdinalIgnoreCase)))
							{
								list.Add(text3);
							}
						}
	...
	public void TraFeature(string sDocFileName, string cfgname, string sstate = "", string IsVirtual = "", int level = 0)
	{
		...
		try
		{
			...
			int num2 = 0;
			bool flag4 = false;
			Sb_Feature.AppendLine("<Node PathName=\"" + code.ToHexString(sDocFileName) + "\" ConfigureName=\"" + code.ToHexString(cfgname) + "\" IsSuppressed=\"" + sstate + "\" ExcludeFromBOM=\"\" DisplayInBOM=\"" + Conversions.ToString(num2) + "\" FeatureName=\"\" >");
			SwDMApplication swDMApplication = swDocMgr;
			int docType = num;
			int num3 = default(int);
			SwDmDocumentOpenError result = (SwDmDocumentOpenError)num3;
			...
			checked
			{
				if (!Information.IsNothing(swDMDocument))
				{
					...
					if (!source.ToList().Exists([SpecialName] (string s) => s.Equals(cfgname, StringComparison.OrdinalIgnoreCase)))
					{
						cfgname = configurationManager.GetActiveConfigurationName();
					}
					if (FilePathNameArr.Count > 0)
					{
						...
						while (true)
						{
							...
							if (((true && FilePathNameArr[num5].Equals(sDocFileName, StringComparison.OrdinalIgnoreCase) && CfgNameArr[num5].Equals(cfgname, StringComparison.OrdinalIgnoreCase)) || (false && FilePathNameArr[num5].Equals(sDocFileName, StringComparison.OrdinalIgnoreCase))) ? true : false)
							{
								flag = true;
					...
					if (!flag)
					{
						...
						CfgNameArr.Add(cfgname);
						CountqutysArr.Add(Conversions.ToString(1));
						Sb_bomdata.AppendLine("PathFileName\u001e\u001c" + sDocFileName);
						Sb_bomdata.AppendLine("ConfigureName\u001e\u001c" + cfgname);
					...
					{
						SwDMConfiguration8 swDMConfiguration = (SwDMConfiguration8)configurationManager.GetConfigurationByName(cfgname);
	...
	public void TraverseAsm_BySWDM(SwDMComponent9 vComponents, int level)
	{
		...
		try
		{
			...
			string text = vComponents.ConfigurationName;
			int num;
			...
			sw.WriteLine(Strings.Space(checked(4 * level)) + Conversions.ToString(level) + "\t" + pathName + "\t" + text + "\t" + Conversions.ToString(vComponents.ExcludeFromBOM) + "\t" + Conversions.ToString(vComponents.IsEnvelope()) + "\t" + Conversions.ToString(vComponents.IsVirtual) + "\t" + vComponents.SelectName);
			SwDMApplication swDMApplication = swDocMgr;
			int docType = num;
			int num2 = default(int);
			SwDmDocumentOpenError result = (SwDmDocumentOpenError)num2;
			...
			SwDMConfigurationMgr configurationManager = swDMDocument.ConfigurationManager;
			string[] source = (string[])configurationManager.GetConfigurationNames();
			if (!source.ToList().Exists([SpecialName] (string s) => s.Equals(text, StringComparison.OrdinalIgnoreCase)))
			{
				text = configurationManager.GetActiveConfigurationName();
			}
			if (num != 2)
			{
				return;
			}
			SwDMConfiguration8 swDMConfiguration = (SwDMConfiguration8)configurationManager.GetConfigurationByName(text);
			object objectValue = RuntimeHelpers.GetObjectValue(swDMConfiguration.GetComponents());
			if (Information.IsNothing(RuntimeHelpers.GetObjectValue(objectValue)))
			{
				return;
